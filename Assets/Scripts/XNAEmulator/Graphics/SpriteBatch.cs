...
*/
        ...
        internal void Draw(Texture2D texture2D, Vector2 position, Nullable<Rectangle> source, Color color, float rotation, Vector2 Origin, float scale, SpriteEffects spriteEffects, float p_3)
        {
			graphicsDevice.DrawQueue.EnqueueSprite(new DrawSpriteCall(texture2D, position, source, color.ToVector4(), Origin, spriteEffects, rotation, scale));
        }
...
*/
			graphicsDevice.DrawQueue.EnqueueSprite(new DrawSpriteCall(texture, Vector2.Zero, source, color.ToVector4(), Vector2.Zero, SpriteEffects.None, rotation));
		}
        ...
        internal void DrawString(SpriteFont font, string value, Vector2 position, Color color, float scale)
        {
            graphicsDevice.DrawQueue.EnqueueString(new DrawStringCall(font, value, position, color.ToVector4(), scale));
        }

        internal void DrawString(SpriteFont font, string value, Vector2 position, Color color, float scale, bool isCentered)
        {
            graphicsDevice.DrawQueue.EnqueueString(new DrawStringCall(font, value, position, color.ToVector4(), scale, isCentered));
        }

        internal void Draw(Texture2D texture2D, Vector2 position, Color color, bool skipViewportScaleTransformation)
        {
            graphicsDevice.DrawQueue.EnqueueSprite(new DrawSpriteCall(texture2D, position, null, color.ToVector4(), Vector2.Zero, SpriteEffects.None, skipViewportScaleTransformation));
        }
    }
}
